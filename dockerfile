...
RUN --mount=type=bind,source=docker,target=/mnt/build --mount=type=bind,source=./pyproject.toml,target=/mnt/ImSwitch/pyproject.toml --mount=type=bind,source=./imswitch/__init__.py,target=/mnt/ImSwitch/imswitch/__init__.py /mnt/build/build-imswitch-deps.sh

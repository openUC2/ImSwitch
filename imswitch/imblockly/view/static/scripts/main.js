...
async function doMicroscopeAction(action_url, payload) {
    try {
        let r = await axios.post(`${ microscope_address}/api/v2/${action_url}`, payload);
        await pollAction(r);
...
function handleConnect() {
    microscope_address = `http://${document.getElementById('ipAddressForm').elements["ip_address"].value}:5000`
    var livePreviewImg = document.getElementById("livePreviewImg")
    runButton.disabled = false;
    stepButton.disabled = false;
    document.getElementById("ofmConnectButton").disabled = false;
    livePreviewImg.onerror = function () {
        alert(`Unable to connect to the OpenFlexure Microscope, please check the Hostname or IP address.`);
        livePreviewImg.src = `images/preview-placeholder.svg`;
        runButton.disabled = true;
        stepButton.disabled = true;
        document.getElementById("ofmConnectButton").disabled = true;
    }
    livePreviewImg.src = `${microscope_address}/api/v2/streams/mjpeg`
}

if (location.protocol === 'https:') {
    alert(`This page was loaded with 'https' and so will not be able to connect to the OpenFlexure Microscope.  Please make sure you connect with 'http'.`)
}
...
function microscopeTest() {
    axios.get(`http://${location.hostname}:5000/api/v2/instrument/configuration`).then((response) => {
        microscope_address = `http://${location.hostname}:5000`;
        try {
            livePreviewImg.src = `${microscope_address}/api/v2/streams/mjpeg`;
            document.getElementById('ipAddressForm').elements["ip_address"].value = location.hostname;
        } catch (err) {
        ...
        try {
            document.getElementById("webAppButton").disabled = false;
        } catch (err) {

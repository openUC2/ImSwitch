...
class LepmonController(LiveUpdatedController):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.wasRunning = self._master.LepmonManager.defaultConfig["wasRunning"]
        self.defaultFlowRate = self._master.LepmonManager.defaultConfig["flowRate"]
        self.defaultNumberOfFrames = self._master.LepmonManager.defaultConfig["numberOfFrames"]
        self.defaultExperimentName = self._master.LepmonManager.defaultConfig["experimentName"]
        self.defaultFrameRate = self._master.LepmonManager.defaultConfig["frameRate"]
        self.defaultSavePath = self._master.LepmonManager.defaultConfig["savePath"]
        self.defaultFileFormat = self._master.LepmonManager.defaultConfig["fileFormat"]
        self.defaultIsRecordVideo = self._master.LepmonManager.defaultConfig["isRecordVideo"]
        self.pumpAxis = self._master.LepmonManager.defaultConfig["axisFlow"]
        self.focusAxis = self._master.LepmonManager.defaultConfig["axisFocus"]
        self.defaultDelayTimeAfterRestart = self._master.LepmonManager.defaultConfig["delayTimeAfterRestart"]
        ...
        if not IS_HEADLESS:
            ...
            # Connect buttons
            self._widget.buttonStart.clicked.connect(self.startLepmonExperimentByButton)
            self._widget.buttonStop.clicked.connect(self.stopLepmonExperimentByButton)
        ...
        if self.wasRunning:
            ...
            self.startLepmonExperiment(timeStamp, experimentName, experimentDescription, 
                                         uniqueId, numImages, volumePerImage, timeToStabilize, delayToStart, 
                                         frameRate, filePath, fileFormat, isRecordVideo)
            
    def startLepmonExperimentByButton(self):
        """ Start Lepmon experiment. """
        ...
        self.startLepmonExperiment(timeStamp = timeStamp, experimentName = experimentName,
    ...
    def startLepmonExperimentFastAPI(self, timeStamp: str, experimentName: str, experimentDescription: str,
    ...
    def startLepmonExperiment(self, timeStamp: str, experimentName: str, experimentDescription: str, 
                                uniqueId: str, numImages: int, volumePerImage: float, timeToStabilize: float, 
                                delayToStart: float=1, frameRate: float=1, filePath: str="./", 
                                fileFormat: str= "JPG", isRecordVideo: bool = True, 
                                pumpSpeed: float = 10000):
    ...
    def stopLepmonExperimentByButton(self):
        """ Stop Lepmon experiment. """
    ...
    def flowExperimentThread(self, timeStamp: str, experimentName: str, 
        ...
        self._logger.debug("Starting the Lepmon experiment thread in {delayToStart} seconds.")
        ...
        while True:
            ...
            if self.is_measure:
                ...
                self.setSharedAttr('Lepmon', _metaDataAttr, metaData)

...
try:
    ...
    print("OffAxisHoloController needs NanoImagingPack to work!")
    isNIP = False
...
class OffAxisHoloController(LiveUpdatedController):
    ...
    - Fresnel propagation after spatial filtering
    ...
    def __init__(self, *args, **kwargs):
        ...
        if not isNIP:
            self._logger.warning("NanoImagingPack not available. Hologram processing disabled.")
            return
    ...
    def _process_offaxis(self, image):
        ...
        mpupil = nip.extract(
            mpupil,
            ROIsize=(int(self._params.cc_radius), int(self._params.cc_radius)),
    ...
    def _process_frame(self, image):
        ...
        try:
            result = self._process_offaxis(image)
    ...
    def set_parameters(self, params: Dict[str, Any]) -> Dict[str, Any]:
        ...
        Example request:
            {"dz": 0.00001, "cc_center": [256, 256], "cc_radius": 80}
        """
        with self._processing_lock:
    ...
    @APIExport(runOnUIThread=True)
    def set_cc_params(self, center: Tuple[int, int], radius: int) -> Dict[str, Any]:
        """Set cross-correlation center and radius for off-axis processing"""
        return self.set_parameters({"cc_center": center, "cc_radius": radius})
    ...
    def mjpeg_stream(self, startStream: bool = True, jpeg_quality: int = 85):
        """
    ...
    def _ensure_camera_running(self):
        ...
        try:
            detector = self._master.detectorsManager[self.camera]
            if not detector.model.isRunning():
    ...
    def updateCCCenter(self):
        ...
        try:
            ...
            self.set_cc_params((centerX, centerY), self._params.cc_radius)
        except Exception as e:

...
Design
------
* Calibrations are keyed by **detector name** (e.g. ``"WidefieldCamera"``).
* On startup the controller reads ``setupInfo.PixelCalibration.affineCalibrations``
  and pushes the derived ``flipX``/``flipY`` and average pixel size into every
  matching ``DetectorManager`` via ``setFlipImage()`` / ``setPixelSizeUm()``.
...
class PixelCalibrationController(LiveUpdatedController):
    ...
    def _getCurrentObjectiveId(self) -> str:
        """Best-effort lookup of the currently selected objective name."""
        try:
            ...
            obj_mgr = getattr(self._master, "objectiveManager", None)
            if obj_mgr is not None and getattr(obj_mgr, "objectiveNames", None):
    ...
    def _loadAffineCalibrations(self):
        ...
        for key, calib_data in self.affineCalibrations.items():
            if key in known_detectors:
                self._applyCalibrationToDetector(key, calib_data)
            else:
                self._logger.info(
                    f"Calibration key '{key}' does not match any active detector - skipping apply"
                )

        for detector_name in known_detectors:
            if detector_name not in self.affineCalibrations:
                self._logger.info(
                    f"No calibration for detector '{detector_name}' - using detector defaults"
                )
    ...
    def getAffineMatrix(self, key: str = None) -> np.ndarray:
        """Return the persisted affine matrix for a detector (or default identity)."""
        if key is None:
            key = self._defaultDetectorName()
        return self._setupInfo.getAffineMatrix(key)

    def getPixelSize(self, key: str = None) -> tuple:
        """Return ``(scale_x, scale_y)`` in µm/px for the given detector."""
        if key is None:
            key = self._defaultDetectorName()
    ...
    def calibrateStageAffine(
        ...
        The result is stored as **pending** and must be reviewed through
        ``getPendingCalibration`` and confirmed via ``applyPendingCalibration``
        (or dropped via ``discardPendingCalibration``) before it takes effect.
        """
        ...
        thread = threading.Thread(
            ...
            args=(detectorName, stepSizeUm, pattern, nSteps, crop_size, isDEBUG),
            daemon=True,
        )
        thread.start()
        return {
    ...
    def _calibrateStageAffineInThread(
        ...
        try:
            ...
            self._logger.info(
                f"Calibration pending approval for detector '{detectorName}'"
            )
            return self._pendingCalibration[detectorName]
        except Exception as exc:
            self._logger.error(
                f"Calibration thread failed for '{detectorName}': {exc}", exc_info=True
            )
            self._pendingCalibration[detectorName] = {
                "error": str(exc),
                "detector_name": detectorName,
    ...
    def getPendingCalibration(self, detectorName: str = None):
        """Return pending (un-applied) calibration data for one or all detectors."""
        if detectorName:
            data = self._pendingCalibration.get(detectorName)
            if data is None:
                return {
                    "success": False,
                    "message": f"No pending calibration for '{detectorName}'",
                }
    ...
    def discardPendingCalibration(self, detectorName: str):
        ...
        if detectorName in self._pendingCalibration:
            ...
            return {
                "success": True,
                "message": f"Pending calibration discarded for '{detectorName}'",
            }
        return {
            "success": False,
            "message": f"No pending calibration for '{detectorName}'",
    ...
    def getCalibratedDetectors(self):
        """Return the list of detector names that have a persisted calibration."""
        return {
            "success": True,
            "detectors": list(self.affineCalibrations.keys()),
        }

    @APIExport()
    def getCalibrationData(self, detectorName: str = "default"):
        """Return persisted calibration data for the given detector."""
        calib = self.affineCalibrations.get(detectorName)
        if calib is None:
            return {
                "success": False,
                "error": f"No calibration found for '{detectorName}'",
            }
        return {
            "success": True,
            "detectorName": detectorName,
    ...
    def setCalibrationData(
        ...
        return {
            "success": True,
            "detectorName": detectorName,
            "message": f"Calibration data saved for '{detectorName}'",
        }

    @APIExport()
    def deleteCalibration(self, detectorName: str):
        """Delete a calibration and reset the detector to defaults."""
        if detectorName not in self.affineCalibrations:
            return {
                "success": False,
                "message": f"No calibration found for '{detectorName}'",
            }

        del self.affineCalibrations[detectorName]
        if (
            self._setupInfo.PixelCalibration is not None
            and detectorName in self._setupInfo.PixelCalibration.affineCalibrations
        ):
            del self._setupInfo.PixelCalibration.affineCalibrations[detectorName]
        try:
            import imswitch.imcontrol.model.configfiletools as configfiletools
            options, _ = configfiletools.loadOptions()
            configfiletools.saveSetupInfo(options, self._setupInfo)
        ...
        return {
            "success": True,
            "detectorName": detectorName,
            "message": f"Calibration deleted for '{detectorName}' (reset to identity)",
        }
    ...
    def manualPixelSizeCalibration(
        ...
        # Preserve previous rotation/flip if a calibration already exists.
        existing = self.affineCalibrations.get(detectorName, {}).get("affine_matrix")
        rotation_deg = 0.0

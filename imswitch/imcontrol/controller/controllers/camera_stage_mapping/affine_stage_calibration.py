"""
Robust automated stage-to-camera affine calibration with per-objective persistence.

This module provides a production-ready calibration system that:
1. Handles camera-stage misalignment (rotation, scaling, potential shear)
2. Works reliably on low-power devices (Raspberry Pi)
3. Provides per-objective calibration storage
4. Offers high precision with sub-pixel accuracy

The calibration determines a 2×3 affine transformation matrix that maps camera 
pixel coordinates to stage micron coordinates for each objective.
...
try:
    ...
    logging.warning("phase_cross_correlation not available, will use FFT-based tracking")

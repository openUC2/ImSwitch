...
TOUPCAM_OPTION_SCANTYPE               = 0x8e       # [RW] Scan Type: 0(linescan), 1(areascan)
...
class Toupcam:
    ...
    @classmethod
    def Version(cls):
        """get the version of this dll, which is: 60.31631.20260606"""
        cls.__initlib()
    ...
    def __initlib(cls):
        if cls.__lib is None:
            # --- ImSwitch patch begin: resolve the native library via the bundled
            # loader (env vars, SDK folder, per-arch bundled libs, system paths)
            # before the stock lookup below. See libloader.py. ---
            try:
                from .libloader import find_toupcam_library
                _libpath = find_toupcam_library()
                if _libpath:
                    if sys.platform == 'win32':
                        cls.__lib = ctypes.windll.LoadLibrary(_libpath)
                    else:
                        cls.__lib = ctypes.cdll.LoadLibrary(_libpath)
            except Exception:
                cls.__lib = None
            # --- ImSwitch patch end ---
            if cls.__lib is None:

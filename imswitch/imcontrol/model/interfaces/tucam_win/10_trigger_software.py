...
class Tucam():
    ...
    def DoSoftwareTrigger(self):
        ...
        # ch:设置相机软触发模式 | en:Set software trigger mode
        TUCAM_Cap_GetTrigger(self.TUCAMOPEN.hIdxTUCam, pointer(m_tgr))
        m_tgr.nTgrMode = m_capmode.TUCCM_TRIGGER_SOFTWARE.value
        m_tgr.nBufFrames = 1
        ...
        for i in range(nTimes):
            try:
                TUCAM_Cap_DoSoftwareTrigger(self.TUCAMOPEN.hIdxTUCam, 0)
                result = TUCAM_Buf_WaitForFrame(self.TUCAMOPEN.hIdxTUCam, pointer(m_frame), 1000)
                print(
                    "Grab the software trigger frame success, index number is %#d, width:%d, height:%#d, channel:%#d, elembytes:%#d, image size:%#d"%(i, m_frame.usWidth, m_frame.usHeight, m_frame.ucChannels,
                    m_frame.ucElemBytes, m_frame.uiImgSize)
                    )
            except Exception:
                print('Grab the software trigger frame failure, index number is %#d'%i)
                continue

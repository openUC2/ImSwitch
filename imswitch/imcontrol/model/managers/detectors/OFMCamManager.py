...
class OFMCamManager(DetectorManager):
    ...
    def __init__(self, detectorInfo, name, **_lowLevelManagers):
        ...
        host = detectorInfo.managerProperties['ofmcam']['cameraHost']
        port = detectorInfo.managerProperties['ofmcam']['cameraPort']
        self._camera = self._getOFMCamObj(host, port)

        model = self._camera.model
        self._running = False
        self._adjustingParameters = False

        for propertyName, propertyValue in detectorInfo.managerProperties['ofmcam'].items():
    ...
    def _getOFMCamObj(self, host, port):
        try:
            from imswitch.imcontrol.model.interfaces.ofmcamera import CameraOFMCam
            self.__logger.debug(f'Trying to initialize OFMCamera {host}')
            camera = CameraOFMCam(host, port)
